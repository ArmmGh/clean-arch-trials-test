...
const ACCEPTED_FILE_TYPES = ['image/png', 'image/jpeg', 'image/gif', 'image/webp', 'image/svg+xml']
...
export default async function uploadArticleImageToIPFSAction(formData: FormData): Promise<any> {
  try {
    ...
    const url = await uploadArticleImageToIPFSUseCase(data)
