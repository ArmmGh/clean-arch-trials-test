...
import { UnauthorizeError, InputParseError, SupabaseError } from '@/entities/errors/common'
...
export default async function unfollowChannelAction(input: z.infer<typeof inputSchema>) {
  try {
    ...
    if (error instanceof InputParseError || error instanceof SupabaseError || error instanceof UnauthorizeError) {
